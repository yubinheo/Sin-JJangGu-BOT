...
class Basics(commands.Cog):
    ...
    async def invite(self, ctx):
        invite = discord.Embed(title = f'{bot.user.name}을 초대하고 싶은가요?', colour = discord.Colour.blue(), timestamp = ctx.message.created_at)
        ...
        outmsg = await ctx.send(embed = invite)
        await outmsg.delete(delay = 30)
        await ctx.message.delete()

    @commands.command(aliases = ['문의'])
    async def question(self, ctx):
        embed = discord.Embed(title = f'{bot.user.name} 공식 디스코드', url = 'https://discord.gg/dn9MbUp',description ='여기로 가면 문의를 할 수 있습니다!', colour = discord.Colour.blue(), timestamp = ctx.message.created_at)
